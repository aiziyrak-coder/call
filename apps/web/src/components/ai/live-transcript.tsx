...
/**
 * Jonli transkripsiya (TZ 5.4). Yakuniy segmentlar lentaga qo'shiladi,
 * qisman natija esa oxirgi qatorni almashtirib turadi.
 */
export function LiveTranscript({ callId }: { callId: string | null }) {
  ...
  // Yangi qator kelganda pastga siljitamiz.
  useEffect(() => {
  ...
  return (
    <Card>
      <CardHeader
        ...
        description={callId ? "Real vaqtda nutqni matnga o'girish" : 'Faol suhbat kutilmoqda'}
        action={
      ...
      <p className="flex items-center gap-1.5 border-t border-[var(--color-border-subtle)] px-5 py-2 text-[11px] text-[var(--color-text-muted)]">
        ...
        Sentiment va tavsiyalar 2-bosqichda shu panelga qo'shiladi
      </p>
    </Card>
  );
}
